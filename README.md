...
1. The repository contains three files, `index.html`, `profile.png` and `deployWebsite.sh`. `index.html` contains an example HTML document. `deployWebsite.sh` contains a console shell script for copying your website files up to your production server and overwriting the default Caddy server home page. Open up and review these files.
